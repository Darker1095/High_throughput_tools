...
def get_result(output_str: str, components: list, cif_name: str):
    ...
    if res["finished"] == 'True':
        ...
        adsorption_heat = output.get_adsorption_heat_infinite_dilution()
        #print("Adsorption Heat Dictionary:", adsorption_heat)
        res["heat_of_adsorption_kJ/mol"] = str(adsorption_heat)
    else:
        res["heat_of_adsorption_kJ/mol"] = ""
    return res
...
def get_field_headers(components: list):
    ...
    units = ['kJ/mol']
    for unit in units:
        headers.append("heat_of_adsorption_" + unit)          
...
def main():
    ...
    with open("./simulation_template.input", "r") as f:
        template = f.read()
    result_file = os.path.join(cur_path, "heat_of_adsorption.csv")
