...
def get_result(output_str: str, components: list, cif_name: str):
    res = {}
    res["name"] = cif_name
    res["He_void_fraction"] = " "
    res["Framework_density_kg/m^3"] = " "
    ...
    if res["finished"] == 'True':
        ...
        for void_fraction in output.get_He_void_fraction():
            res["He_void_fraction"] = void_fraction
        for density in output.get_Framework_density():
            res["Framework_density_kg/m^3"] = density        

    return res


def get_field_headers(components: list):
    headers = ["name", "finished", "He_void_fraction","Framework_density_kg/m^3"]
...
def main():
    ...
    with open("./simulation_template.input", "r") as f:
        template = f.read()
    result_file = os.path.join(cur_path, "He_void_fraction_density_results.csv")
